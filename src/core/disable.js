...
module.exports = {
	...
	async execute(bot, messageCreate, args, prefix){
		...
		if(disabledCategory === null && args[0] === "category") guildconfigdb.set(`disabledCategory_${messageCreate.guild.id}_${args[1]}`, "enabled")
		...
		if(args[0] === "category" && cats.includes(args[1])) guildconfigdb.set(`disabledCategory_${messageCreate.guild.id}_${args[1]}`, "disabled").then(()=> messageCreate.channel.send({ embeds: [categoryEmbed] }))
		if(args[0] === "command" && !cmd) return messageCreate.channel.send(reject.user.args.invalid)
		if(args[0] && cmd) guildconfigdb.set(`disabled_${messageCreate.guild.id}_${cmd}`, "disabled").then(()=> messageCreate.channel.send({ embeds: [commandEmbed] }))
	}
}
