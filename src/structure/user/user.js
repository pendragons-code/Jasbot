...
module.exports = {
	...
	async execute(bot, messageCreate, args) {
		db.add(`cmd_${messageCreate.author.id}`)
		...
		if(newUser === 0){
			...
			embed.setFooter({ text: "Hello there! I am a new bot that the owndr made out of complete boredom for some reason." })
			embed.setTimestamp()
			embed.setTitle(`Hoiya ${messageCreate.author.username}!`)
			try{
				messageCreate.author.send({ embeds: [embed] })
				db.set(`first_${messageCreate.author.id}`)
			}catch(e){
